...
const Bookings = () => {
  ...
  useEffect( () => {
    ...
    setload(true);
    try {
    const response = await BookingService.getallbooking(location.state.name);
  ...
  <table class="w-full border-collapse bg-white text-left text-sm text-gray-500">
    ...
    <tbody class="divide-y divide-gray-100 border-t border-gray-100">
      ...
      <tr class="hover:bg-gray-100">
        ...
        <td class="px-6 py-4">
          <span
            class={`inline-flex items-center gap-1 rounded-full  ${(mybookings.status==="completed") ? 'bg-green-50 text-green-600':(mybookings.status==="Not completed")?'bg-purple-50 text-purple-600':'bg-red-50 text-red-500'} px-2 py-1 text-xs font-semibold `}
        ...
        <td class="px-7 py-4 ml-auto ">
          <div class="flex  gap-4">
            <button className='p-2' onClick={() => navigate("/edit",{state:{username:location.state.name}})}>Edit</button>
